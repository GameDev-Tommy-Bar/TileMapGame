    ...
    bool possValid(Vector3 pos, Tilemap map)
    {
        ...
        while (attempts < maxAttempts)
        {
           ...
           // Debug.Log("unique count: "+uniqeTiles.Count);
            ...
            {
                Debug.Log(attempts+") path from "+startNode +"to "+endNode+" is in size "+shortestPath.Count+" tiles count "+counter+" set_size "+set.Count);
                return true;
            }
            else if(shortestPath.Count > 0)
            {
                ...
                Debug.Log(attempts+") shortest path size from "+startNode+ "to "+endNode+ "is  "+shortestPath.Count+" tiles count " +counter+" set_size "+set.Count);
            }
            else if(shortestPath.Count == 0){
                Debug.Log(attempts+") NoPath from "+startNode+ "to "+endNode+ ",shortestpath count =  "+shortestPath.Count+" tiles count "+counter+" set_size "+set.Count);
            }
